...
**In order to format my results more nicely,** I should copy them into Excel first, format them, and then copy them into Word. There are also some newer ways to make nicely formatted tables in Stata.
...
It's also worth trying out the `lean1` and `lean2` schemes (type `findit lean2` to install). In new versions of Stata, the new default scheme `stcolor` provides visually appealing graphs.
